...
# 实现状态-动作Q函数（Q值）的神经网络，它输入一个状态和一个动作参数向量，然后输出一个Q值，以表示在给定状态下执行该动作对预期回报的估计
from DRL.agents.agent import Agent
from DRL.agents.memory.memory import NStepReplayBuffer, ReplayBuffer


class QActor(nn.Module):

    def __init__(self, state_size, action_size, action_parameter_size, lstm_hidden_size=64, lstm_num_layers=2,
        ...
        last_hidden_layer_size = lstm_hidden_size

        if hidden_layers is not None:
            nh = len(hidden_layers)
            self.layers.append(nn.Linear(last_hidden_layer_size, hidden_layers[0]))
            for i in range(1, nh):
    ...
    def forward(self, state, action_parameters):
        ...
        # Ensure LSTM input is 3D (batch_size, seq_length, input_size)
